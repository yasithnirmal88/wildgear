      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <header className="glass-header" style={{ 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <h1 className="playfair" style={{ margin: 0, fontSize: isMobile ? 16 : 20, fontWeight: 900, color: "#1B4332", textTransform: 'uppercase', letterSpacing: '-0.03em' }}>{tab.toUpperCase()}</h1>
          </div>
        ...
        <div style={{ padding: isMobile ? 16 : 32, flex: 1 }}>
                ...
                <div key={item.id} style={{ background: "#fff", borderRadius: 24, border: "1px solid #EDE8E0", overflow: "hidden", display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ padding: isMobile ? 12 : 20, flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 'auto' }}>
                      <Btn onClick={() => openQtyModal(item)} style={{ flex: 1, fontSize: 10, padding: '6px 4px', justifyContent: 'center', background: '#52796F', color: '#F8F5F0', border: 'none' }}>STOCK</Btn>
                      <Btn onClick={() => { setItemForm(item); setEditItemId(item.id); setModal("item"); }} style={{ flex: 1, fontSize: 10, padding: '6px 4px', justifyContent: 'center', background: '#52796F', color: '#F8F5F0', border: 'none' }}>EDIT</Btn>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: isMobile ? 500 : 'auto' }}>
                  ...
                  <tbody>
                    ...
                    <tr key={c.id} style={{ borderBottom: "1px solid #F8F5F0" }}>
                      <td style={{ padding: 16 }}><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><Avatar name={c.name} size={32} /><div style={{ fontWeight: 700, fontSize: 14, color: '#1B4332' }}>{c.name}</div></div></td>
          ...
          {tab === "billing" && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              <div style={{ background: 'linear-gradient(135deg, #1B4332 0%, #2D6A4F 100%)', borderRadius: 24, padding: 32, color: '#fff', boxShadow: '0 20px 40px rgba(27,67,50,0.15)' }}>
              ...
              <div style={{ background: "#fff", borderRadius: 24, border: "1px solid #EDE8E0", overflow: "hidden", boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid #F8F5F0', fontWeight: 900, fontSize: 15, color: '#1B4332', display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <tbody>
                      {rentals.map(r => (
                        <tr key={r.id} className="admin-table-row" style={{ borderBottom: "1px solid #F8F5F0" }}>
                          ...
                          <td style={{ padding: 16, fontWeight: 800, fontSize: 14, color: '#1B4332' }}>LKR {r.totalAmount.toLocaleString()}</td>
